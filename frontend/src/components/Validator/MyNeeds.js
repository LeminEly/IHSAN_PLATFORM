        ...
        {needs.map((need) => (
          <div key={need.id} className="bg-white rounded-lg shadow p-6">
            <div className="flex justify-between items-start">
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(need.status)}`}
              >
                {need.status}
              </span>
            </div>
            ...
            {need.beneficiary && (
              <div className="mt-4 pt-4 border-t">
                <p className="text-sm text-gray-600">
                  ...
                  {need.beneficiary.description}
                  {need.beneficiary.family_size &&
                    ` (Famille de ${need.beneficiary.family_size} personnes)`}
                </p>
              </div>
            )}
          </div>
