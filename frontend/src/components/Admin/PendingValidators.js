...
function PendingValidators() {
  ...
  const loadValidators = async () => {
    setLoading(true);
    try {
      const r = await admin.getPendingValidators();
      setValidators(r.data || []);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleAction = async (id, type) => {
    const reason = window.prompt(
      type === "approve" ? "Raison (optionnelle) :" : "Raison du rejet :",
    );
  ...
  return (
    <div style={{ maxWidth: "860px" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        >
          {validators.length} demande{validators.length !== 1 ? "s" : ""} en
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {v.user?.full_name?.[0]?.toUpperCase() || "V"}
                  </div>
                  <div>
                    <div
                      style={{
