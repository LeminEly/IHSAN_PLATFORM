...
function Navbar() {
  ...
  const roleColors = { donor: 'badge-blue', validator: 'badge-purple', partner: 'badge-gold', admin: 'badge-red' };
  const roleLabels = { donor: 'Donneur', validator: 'Validateur', partner: 'Partenaire', admin: 'Admin' };
  ...
  return (
    <>
      ...
      <nav style={{
        ...
        <div className="container mx-auto px-6" style={{ maxWidth: '1280px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
            ...
            {/* Right desktop — inchangé */}
            <div className="nav-desktop-right" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              {user ? (
                <>
                  ...
                  <button onClick={handleLogout} style={{
                    padding: '6px 14px', borderRadius: 8, fontSize: '0.8125rem', fontWeight: 500,
      ...
      {/* Drawer mobile */}
      <div ref={menuRef} className={`nav-mobile-drawer${menuOpen ? ' open' : ''}`}>
        ...
        <div className="nav-drawer-header">
          <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{
              ...
              <span style={{ color: 'var(--gold-l)', fontSize: 11, fontFamily: "'Amiri', serif", fontWeight: 700 }}>إ</span>
            </div>
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 700, fontSize: '1.125rem', color: 'var(--g-800)' }}>IHSAN</span>
          </Link>
        ...
        {user && (
          <div className="nav-drawer-user">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--ink)' }}>{user.full_name}</div>
                <span className={`badge ${roleColors[user.role] || 'badge-gray'}`} style={{ fontSize: '0.6rem' }}>
                  {roleLabels[user.role] || user.role}
                </span>
              </div>
        ...
        <div className="nav-drawer-links">
          {links.length > 0 && (
            <>
              ...
              {links.map(link => (
                <Link key={link.to} to={link.to} className={`nav-drawer-link${isActive(link.to) ? ' active' : ''}`}>
                  {link.label}
                </Link>
              ))}
            </>
          ...
          {!user && (
            <>
              ...
              <Link to="/login"    className="nav-drawer-link">Se connecter</Link>
              <Link to="/register" className="nav-drawer-link">S'inscrire</Link>
            </>
          )}
          <div style={{ borderTop: '1px solid rgba(0,0,0,.05)', marginTop: 8, paddingTop: 8 }}>
            <Link to="/" className="nav-drawer-link">Accueil public</Link>
          </div>
        </div>
